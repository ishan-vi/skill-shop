...
<script>
    ...
    window.addEventListener("resize", function() {
        itemsPreview = window.innerWidth >= 768 ? 3 : 1;
        maxSlides = Math.ceil(totalItems / itemsPreview) - 1;
        if (currentSlide > maxSlides) {
            currentSlide = maxSlides;
        }
        updateCarousel();
        
        // Recreate dots for responsive layout
        createDots();
    });
    ...
    function createDots() {
        ...
        for (var i = 0; i <= maxSlides; i++) {
            ...
            dot.className = `carousel-dot w-3 h-3 rounded-full transition-all ${i == currentSlide ? "bg-blue-600" : "bg-gray-300"}`;
